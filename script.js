...
function actualizarDatosAleatorios() {
    vuelos.forEach(vuelo => {
        ...
        if (probabilidadCambioEstado < 0.3) {
...
$(document).ready(function() {
    $('#estado-vuelo-btn').click(function(event) {
        ...
        // Mostrar/Ocultar el contenedor de detalles de vuelos
        $('#detalle-vuelos').toggle();

        // Si el contenedor está visible, actualizar la cartelera
...
setInterval(() => {
    rotarArray(vuelos); // Rota el array
    actualizarDatosAleatorios(); // Actualiza los datos aleatorios
    if ($('#detalle-vuelos').is(':visible')) {
        actualizarCartelera(); // Actualiza la cartelera si está visible
    }
}, 1000);
